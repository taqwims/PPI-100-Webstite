...
import { useSchoolBank } from '../../hooks/useFeature';
...
/** Reusable bank info component — renders dynamic bank data from feature store */
const BankAccountInfo: React.FC = () => {
    ...
    return (
        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 space-y-3">
            <p className="text-xs text-blue-600 font-medium">Transfer ke rekening:</p>
    ...
    const handleMultiPayment = async () => {
        ...
        try {
            ...
            if (multiPayMethod === 'Midtrans') {
                ...
                toast.success(`Berhasil mengirim permintaan pembayaran ${selectedBillIds.length} tagihan. Silakan hubungi admin untuk verifikasi.`);
                setSelectedBillIds([]);
                setShowMultiPayModal(false);
                refetch();
            }
                                    ...
                                    <tr className="bg-white text-slate-400 border-b border-slate-200 text-[10px] uppercase tracking-widest font-bold">
                                        <th className="p-4 w-10">
                                            <input
                                                ...
                                                checked={studentBills.length > 0 && studentBills.filter((b: any) => b.status !== 'Paid' && !hasPendingTransfer(b)).every((b: any) => selectedBillIds.includes(b.id)) && studentBills.some((b: any) => b.status !== 'Paid' && !hasPendingTransfer(b))}
                                                onChange={(e) => {
                                                    const availableIds = studentBills.filter((b: any) => b.status !== 'Paid' && !hasPendingTransfer(b)).map((b: any) => b.id);
                                                    if (e.target.checked) {
                                                        setSelectedBillIds(prev => Array.from(new Set([...prev, ...availableIds])));
                                                    } else {
                                                        setSelectedBillIds(prev => prev.filter(id => !availableIds.includes(id)));
                                                    }
                                                }}
                                            />
                                        </th>
            ...
            {showMultiPayModal && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-indigo-50">
                            <h2 className="text-lg font-bold flex items-center text-slate-800">
                                <CreditCard className="text-indigo-600 mr-2" size={22} /> Bayar Kolektif Terpilih
                            </h2>
                        ...
                        <div className="p-6 space-y-6">
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                                    ...
                                    {selectedBills.map((b: Bill) => (
                                        <div key={b.id} className="flex justify-between items-center text-sm bg-white p-2.5 rounded-lg border border-slate-100 shadow-sm">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] text-slate-400">{b.student?.user?.name}</span>
                                            </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-slate-200 flex justify-between items-center">
                                    <span className="text-sm font-bold text-slate-600">Total Harus Dibayar</span>
                            ...
                            <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 italic text-center text-xs text-blue-700">
                                💡 Pembayaran multi-tagihan akan dicatat sebagai satu kali transaksi pembayaran kolektif.
                            </div>

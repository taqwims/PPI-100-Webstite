...
export const hasPendingMidtrans = (bill: Bill) => {
    return bill.status !== 'Paid' && (bill.payments || []).some(p => (p.payment_method === 'Midtrans' || p.payment_method === 'Xendit' || p.payment_method === 'Mayar') && p.status === 'Pending' && p.transaction_id);
};
